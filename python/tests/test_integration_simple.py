...
Covers all major Client API capabilities:
  ...
  - execute_many  (parallel batch, mixed results, drop_failed)
...
class TestListTheoryCommands:
    ...
    def test_proof_stmts_filter_returns_only_lemmas(self, client, hol_session):
        ...
        proof_cmds = client.list_theory_commands(hol_session, SIMPLE_THY, only_proof_stmts=True)
    ...
    def test_proof_stmts_contains_all_four_lemmas(self, client, hol_session):
        client.load_theory(hol_session, SIMPLE_THY)
        cmds = client.list_theory_commands(hol_session, SIMPLE_THY, only_proof_stmts=True)
        texts = [c.text for c in cmds]
        for name in ("trivial", "add_comm_nat", "conj_easy", "nat_not_zero"):
            assert any(name in t for t in texts), f"lemma {name!r} not found in commands"
    ...
    def test_lemma_line_numbers_match_source(self, client, hol_session):
        ...
        assert by_name["trivial"].line   == LINE_TRIVIAL,  f"trivial at line {by_name['trivial'].line}"
...
class TestInitState:
    def test_by_line_yields_proof_mode(self, client, hol_session):
        client.load_theory(hol_session, SIMPLE_THY)
        state = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_TRIVIAL).unwrap()
        try:
    ...
    def test_by_line_and_after_command_reach_same_lemma(self, client, hol_session):
        ...
        client.load_theory(hol_session, SIMPLE_THY)
        s_line = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_TRIVIAL).unwrap()
    ...
    def test_different_lemmas_produce_different_states(self, client, hol_session):
        client.load_theory(hol_session, SIMPLE_THY)
        s1 = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_TRIVIAL).unwrap()
        s2 = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_ADD_COMM).unwrap()
        try:
    ...
    def test_error_result_contains_failed_line_and_last_success(self, client, hol_session):
        """init_state on a line past the end of the file returns a clean error."""
        client.load_theory(hol_session, SIMPLE_THY)
        # Force a failure by using a deliberately broken theory path
        # (smoke-level: just verify the InitStateResult API is consistent)
        result = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_TRIVIAL)
        # Happy path object is still an InitStateResult
        assert result.is_success()
        assert result.success is not None
        assert result.error is None
        client.drop_state([result.success.state_id])
...
class TestExecute:
    ...
    def test_apply_rule_yields_success_not_complete(self, client, hol_session):
        ...
        client.load_theory(hol_session, SIMPLE_THY)
        state = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_CONJ).unwrap()
        try:
...
class TestIncludeText:
    ...
    def test_execute_include_text_true_returns_proof_text(self, client, hol_session):
        ...
        state = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_CONJ).unwrap()
        try:
            result = client.execute(state.state_id, "apply (rule conjI)", include_text=True)
    ...
    def test_execute_include_text_false_returns_empty(self, client, hol_session):
        ...
        state = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_CONJ).unwrap()
        try:
            result = client.execute(state.state_id, "apply (rule conjI)", include_text=False)
...
class TestRunSledgehammer:
    ...
    def test_sledgehammer_on_add_comm_nat(self, client, hol_session):
        ...
        client.load_theory(hol_session, SIMPLE_THY)
        state = client.init_state(hol_session, SIMPLE_THY, after_line=LINE_ADD_COMM).unwrap()
        try:
            found, tactic, result = client.run_sledgehammer(
                state.state_id,
                timeout_ms=120_000,
